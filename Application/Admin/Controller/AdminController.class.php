...
class AdminController extends Controller {
	...
	public function zblb(){
		...
		$web = C('WEB');
		$this->assign('web',$web);
		$arr = M('video')->where($where)->page($_GET['p'].',15')->order("v_id desc")->select();
		$this->assign('arr',$arr);
	...
	public function get_host(){
		...
		$count = M('host')->where(['id'=>$hid])->count();
		if($count == 0){
			$data = array(
				'result'=>'0',
				'msg'=>'没有这个职业',
		...
		$host = M('zcr')->join('hl_host on hl_host.id = hl_zcr.hid')->field($field)->where(['hid'=>$hid])->select();
		$data = array(
			'result'=>'1',
			'msg'=>'',
			'data'=>$host,
	...
	public function edit_zb(){
		...
		$hosts = M('zcr')->join('hl_host on hl_zcr.hid = hl_host.id')->field($field)->where(['zcr_id'=>array('in',$host_data)])->select();
		$this->assign('zcrs',$hosts);
		...
		if(!$info){
			...
			//$this->error($upload->getError());
		    		...
		    		foreach($info as $ink=>$inv){
		    			$name=substr($inv['savepath'],2).$inv['savename'];
		    			$data[$ink] = $name;
		    		}
	...
	public function bczb(){
		...
		if($_FILES['zb_img']['name'] || $_FILES['watermark']['name']){
	...
	public function save_zcr(){
		...
		$data['zcr_star'] =   I('post.num','strip_tags');
		$data['zcr_url'] =  $_POST['url'];
		$data['hid'] = $_POST['hid'];
		//print_r($data);exit;
		// print_r($data);exit;
		$year = date('Y',time());
	...
	public function hostlb(){
		...
		$p = I('get.p')?I('get.p'):1;
		$host = M('host');
		$hosts = $host->order('id desc')->page($p.',10')->select();
		$count = $host->count();
		if($count != 0){
			foreach($hosts as $hk=>$hv){
				$hosts[$hk]['date'] = date('Y-m-d H:i:s',$hv['pubtime']);
			}
		}
		$Page  = new \Think\Page($count,10);// 实例化分页类 传入总记录数和每页显示的记录数
		//$Page -> setConfig('header','共%TOTAL_ROW%条');
	...
	public function host_add(){
		...
		if(!IS_POST){
			...
			$ok = M('host')->add($_POST);
			if($ok){
				$this->redirect('Admin/Admin/hostlb');
			}else{
				$this->error('数据库异常');
			}
		}
	...
	public function host_del(){
		...
		$count = M('host')->where(['id'=>$id])->count();
		if($count == 0){
			$this->error('数据库没有数据');
		}

		$ok = M('host')->where(['id'=>$id])->delete();
		if($ok){
			$this->redirect('Admin/Admin/hostlb');
		}else{
			$this->error('参数错误');
	...
	public function host_edit(){
		...
		if(!IS_POST){
			...
			$ok = M('host')->where(['id'=>$id])->save($_POST);
			if($ok !== false){
				$this->redirect('Admin/Admin/hostlb');
			}else{
				$this->error('数据库异常');
	...
	public function zcrlb(){
		...
		if(session('admintype') == 1){
				...
				$list = $model->join('hl_host on hl_host.id = hl_zcr.hid')->field($field)->page($_GET['p'].',15')->select();
	...
	public function jxsplb(){
		...
		$where['delete_at'] = 0;
		$field = "*";
		$list = $model->field($field)->where($where)->page($_GET['p'].',15')->order("jx_id desc")->select();
		$this->assign('res',$list);// 赋值数据集
	...
	public function jxsp_del(){
       if(!session('adminid')){
		...
		$ok = M('jx_video')->where($where)->save(['delete_at'=>1]);
		if($ok){
		...
		if(session('adminid') && session('admintype') == 1){
			if(!IS_POST){
				...
				if($_FILES['big_img']['name'] || $_FILES['small_img']['name']){
					...
					unlink(__ROOT__.$lunbo['big_img']);
					unlink(__ROOT__.$lunbo['small_img']);
        ...
        if (session('admintype') == 1) {
            ...
            if (IS_POST) {
                ...
                if (!$info) {
                	...
                	$gg=M("guanggao");
                    if ($data['fenlei'] == 1 || $data['fenlei'] == 2 || $data['fenlei'] == 3 || $data['fenlei'] == 4 || $data['fenlei'] == 5 || $data['fenlei'] == 7) {
                        foreach ($info as $key => $file) {
                            //print_r($file);exit;
                            $name = substr($file['savepath'], 2) . $file['savename'];
                            $data[$key] = $name;
                        ...
                        $ok = $gg->add($data);
                    }elseif ($data['fenlei'] == 6) {
						for ($i=1;$i<=5;$i++){
							$data['small_img']=false;
                            $data['big_img']=false;
                            foreach ($info as $key => $file) {//1
								$name = substr($file['savepath'], 2) . $file['savename'];
								if($key == 'big_img'.$i){
										$data['big_img']=$name;
										if($key == 'small_img'.$i){
							...
							if ($data['big_img']===false && $data['small_img']===false){
                                continue;
                            }else{
                                $ok = $gg->add($data);
                            }
...
/*	public function add_gg(){
	...
	public function zflb(){
		...
		$arr = M('video')->where($where)->order("v_id desc")->page($_GET['p'].',10')->select();
